    ...
    deleg_timeout(0)
{
  ...
  if(res < 0)
    ldout(cct, 0) << " _SPEEDYMETA " << "create speedymeta_count_phd_accuracy thread error." << dendl;
  pthread_detach(tid_sm_ms);
  #endif
			 ...
			 bufferlist *pdirbl)
{
  ...
  #ifdef _SPEEDYMETA_MEASUREMENT
  op_metaio++;
  // ldout(cct, 0) << " [ MetaIO ] "<< tid << " lat " << lat << dendl;
  #endif
		      ...
		      Inode **out, const UserPerm& perms)
{
  ...
  *out = in.get();
  ...
  if(in && in->prefetched && !in->accessed){
    ldout(cct, 3) << " _SPEEDYMETA_MEASUREMENT " << " access " << name << dendl;
...
int Client::ll_removexattr(Inode *in, const char *name, const UserPerm& perms)
{
  ...
  #ifdef _SPEEDYMETA
    if(strncmp(name, "user.dc.", 8) == 0){
      // remove xattrs(data correlations) in client buffer and return
      speedymeta_cache_rm_dc(in, name + 8);
      return 0;
    }
  #endif
