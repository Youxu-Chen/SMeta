...
public:
  ...
  int speedymeta_prefetch(MDRequestRef& mdr, CInode *ref);
  ...
  #endif
  #ifdef _SPEEDYMETA_AC
  std::unordered_map<string, std::vector<string>> list_ac;
  void _init_ac();
  int speedymeta_prefetch_ac(MDRequestRef& mdr, CInode *ref);
  #endif
private:
